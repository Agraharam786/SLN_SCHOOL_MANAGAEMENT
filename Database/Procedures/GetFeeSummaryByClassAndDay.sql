...
GO
DROP PROCEDURE IF EXISTS GetFeeSummaryByClassAndDay
GO
CREATE PROCEDURE GetFeeSummaryByClassAndDay
AS
BEGIN
    ...
    SET @cursor = CURSOR FOR
        ...
        FROM SLN_FEE
        WHERE IS_DELETED = 0
        AND PAID_DATE >= CAST(GETDATE() AS DATE);  -- Limit to today
    ...
    SET @sql = @sql + ' ,
        ...
        FROM SLN_FEE
        WHERE PAID_DATE >= CAST(GETDATE() AS DATE)  -- Limit to today
        AND IS_DELETED = 0
    ...
    SET @sql = @sql + ',
        ...
        FROM SLN_FEE
        WHERE PAID_DATE >= CAST(GETDATE() AS DATE)
        AND IS_DELETED = 0
    )
    SELECT
        Weekday,
